...
Сборка `GOOS=wasip1 GOARCH=wasm` (требуется Go 1.21+).
...
Требуют решения владельца проекта до соответствующей вехи. До получения ответа реализуется самый простой вариант с пометкой `// SPEC-AMBIGUITY:`.

1. **Имя и расширение формата.** В документе используется `.vui`. Альтернативы: `.vtd`, `.dlg`. Влияет на M0.
2. **Человекочитаемый синтаксис поверх JSON.** JSON канонический и машинный. Стоит ли дать авторский синтаксис (TOML-подобный или собственный), компилирующийся в тот же JSON? Не раньше M2.
3. **Область видимости ID.** Предложено: уникальность в пределах фрейма. Нужна ли глобальная адресация между фреймами?
4. **Многострочный редактор.** В README он не перечислен среди виджетов, но `EditorView` упоминается в ARCHITECTURE.md. Входит ли он в словарь M0?
5. **Локализация.** Выносить ли строки из `.vui` в отдельные каталоги сообщений, и если да — в каком формате?
6. **Минимальная поддерживаемая версия Go.** wasip1 требует 1.21+; хочется ли держать планку ниже для основной библиотеки?
7. **Лицензия сгенерированного кода.** BSD-3-Clause как у ядра, или явное исключение для вывода `vuic`?
