...
Для ситуации, когда мы по какой-то причине не смогли зацепиться за PTY (скажем, мы в
Wine, где это запрещено, потому что от ConPTY там пока только заглушка),
нужны **два стиля консоли** (переключаются в настройках), общие для всех режимов исполнения:
