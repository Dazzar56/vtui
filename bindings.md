...
| **M1 / A7** | Модель данных по запросу `RowProvider` для `ScrollView`/`ListBox`/`Table` | *В процессе* |
| **M1 / A8** | Гарантированное восстановление терминала `Shutdown` | *К выполнению* |
| **M2** | Движок раскладки (раздел 7), `LoadDialog`, `vuic`, `vtui-lint`, golden-тест | *К выполнению* |
...
## M1 — Изменения ядра [В ПРОЦЕССЕ: A1-A6 ГОТОВЫ]
...
- [ ] **A7:** Модель данных «по запросу» `RowProvider` для `ScrollView`/`ListBox`/`Table`.
- [ ] **A8:** Контракт гарантированного восстановления терминала `Shutdown`.
